...
func (b *networkBridge) getUnicastChannel(
	...
	for i := 0; i < retryCount+1; i++ {
		...
		logger.Warningf("failed to get unicast channel with peer [%v] because of:[%v];"+
			"will retry after wait time", peerTransportID.String(), err)

		time.Sleep(retryWaitTime)
	}

	if err == nil {
		err = fmt.Errorf("unknwon error")
	}
...
func (b *networkBridge) sendTo(
	...
	if err != nil {
		return fmt.Errorf("[m:%x]: failed to find unicast channel for [%v]: [%v]",
			b.groupInfo.memberID, receiverTransportID, err)
	}

	if err := unicastChannel.Send(message); err != nil {
		return fmt.Errorf("[m:%x]: failed to send unicast message: [%v]",
			b.groupInfo.memberID, err)
	}
...
func (b *networkBridge) registerProtocolMessageHandler(
	...
	b.tssMessageHandlersMutex.Unlock()
}
