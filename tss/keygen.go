...
// it is possible to generate the parameters way ahead of the tss protocol execution.
func GenerateTSSPreParams(
	...
	if err != nil {
		return nil, fmt.Errorf("failed to generate tss per-params: [%v]", err)
	}
...
// member represents an initialized member who is ready to start distributed key generation.
type member struct {
	*groupInfo
	...
	// channel where a result of the key generation protocol execution will be written to.
	keygenEndChan <-chan keygen.LocalPartySaveData
}

// generateKey executes the protocol to generate a signing key. this function
// needs to be executed only after all members finished the initialization state.
...
func (s *member) generateKey(ctx context.Context) (*ThresholdSigner, error) {
	if err := s.keygenParty.Start(); err != nil {
		return nil, fmt.Errorf("failed to start key generation: [%v]", s.keygenParty.WrapError(err))
	}
	...
	for {
		...
		case <-ctx.Done():
			...
			return nil, timeoutError{
				timeout:   KeyGenerationProtocolTimeout,
...
func initializeKeyGenerationParty(
	...
	if err := networkBridge.connect(
		...
		return nil, nil, fmt.Errorf("failed to connect network bridge: [%v]", err)
	}
