...
func (ucm *unicastChannelManager) handleIncomingStream(stream network.Stream) {
	logger.Debugf("[%v] processing incoming stream [%v] from peer [%v]",
		ucm.identity.id, stream.Protocol(), stream.Conn().RemotePeer())

	channel, isExistingChannel, err := ucm.getUnicastChannel(stream.Conn().RemotePeer())
	if err != nil {
		logger.Errorf("[%v] incoming stream [%v] from peer [%v] dropped: [%v]",
			ucm.identity.id, stream.Protocol(), stream.Conn().RemotePeer(), err)
		return
	}
...
func (ucm *unicastChannelManager) getUnicastChannelWithHandshake(
	...
	if err := ucm.trialHandshake(peerID); err != nil {
...
func (ucm *unicastChannelManager) trialHandshake(peerID net.TransportIdentifier) error {
	...
	hasConnectionWithPeer := ucm.p2phost.Network().Connectedness(remotePeer) == network.Connected
