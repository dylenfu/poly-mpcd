...
import (
	...
	"github.com/keep-network/go-libp2p-bootstrap"
...
const (
	...
	// DefaultConnMgrLowWater is the default value for the connection managers
	// 'low water' mark
...
const (
	...
	FirewallCheckTick = 10 * time.Minute

	// ConnectedPeersCheckTick is the amount of time between periodic checks of
	// the number of connected peers.
	ConnectedPeersCheckTick = 1 * time.Minute
)

// MPCD network protocol identifiers
const (
...
func (p *provider) BroadcastChannelFor(name string) (
...
func (p *provider) BroadcastChannelForwarderFor(name string) {
	...
	if err := p.broadcastChannelManager.newForwarder(name, timeout); err != nil {
		logger.Warningf("could not create message forwarder for channel [%v]: [%v]", name, err)
	}
}
...
func (cm *connectionManager) GetPeerPublicKey(connectedPeer string) (*key.NetworkPublic, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to decode peer ID from [%s]: [%v]", connectedPeer, err)
	}

	peerPublicKey, err := peerID.ExtractPublicKey()
	if err != nil {
		return nil, fmt.Errorf("failed to extract peer [%s] public key: [%v]", connectedPeer, err)
	}
...
func Connect(
	...
	if config.DisseminationTime < 0 || config.DisseminationTime > MaximumDisseminationTime {
		return nil, fmt.Errorf("dissemination time mut be in range [0, %v]", MaximumDisseminationTime)
	}
...
func discoverAndListen(
	...
	if err != nil {
		return nil, fmt.Errorf("could not create authenticated transport: [%v]", err)
	}
	...
	if addresses := parseMultiaddresses(announcedAddresses); len(addresses) > 0 {
		addressFactory := func(addrs []ma.Multiaddr) []ma.Multiaddr {
			logger.Debugf("replacing default announced addresses [%v] with [%v]", addrs, addresses)
			return addresses
		}
...
func parseMultiaddresses(addresses []string) []ma.Multiaddr {
	...
	for _, address := range addresses {
		...
		if err != nil {
			logger.Warningf("could not parse address string [%v]: [%v]", address, err)
			continue
		}
...
func (p *provider) bootstrap(
	...
	// todo(fuk): use the io.closer to shutdown the bootstrapper when we build out a shutdown process
...
func buildNotifiee() libp2pnet.Notifiee {
	...
	notifyBundle.ConnectedF = func(_ libp2pnet.Network, connection libp2pnet.Conn) {
		logger.Infof("established connection to [%v]",
	...
	notifyBundle.DisconnectedF = func(_ libp2pnet.Network, connection libp2pnet.Conn) {
		logger.Infof("disconnected from [%v]",
			multiaddressWithIdentity(
				connection.RemoteMultiaddr(),
				connection.RemotePeer(),
			),
